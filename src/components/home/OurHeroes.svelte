<script lang="ts">
import type { CcDataHero } from "@payloadTypes"
import { fade, fly } from "svelte/transition"
import Icon from "../Icon.svelte"
import Headline from "../_fragments/Headline.svelte"

export let content: Array<CcDataHero>

let position = 0
$: currentHero = content[position]
...
<section class="w-full pt-96 lg:mb-24 lg:pt-16">
    {#if content.length}
        {#key currentHero}
            {#if currentHero.image && typeof currentHero.image !== "number"}
                <img
            ...
            {#if content.length}
                {#key currentHero.name}
                    ...
                    >
                        ...
                        <p class="font-bold">{currentHero.summary}</p>
                        <p>{@html currentHero.story}</p>
                    </div>
                {/key}
            {:else}
                <p>No heroes provided.</p>
            {/if}
        </div>
    </div>
</section>
